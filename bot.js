...
client.on('ready', function(){
    ...
    setInterval(function (){
        ...
        i = i+j;
        client.user.setGame(setGame[i],`http://www.twitch.tv/6546yftd`);
    }, ms);100000
...
if (message.content === '_inv') {
      message.author.send('https://discordapp.com/api/oauth2/authorize?client_id=520928924591718400&permissions=8&scope=bot').catch(e => console.log(e.stack));
...
client.on("guildMemberAdd", member => {
        if(member.guild.id === "520940148620263424") {  // ايدي السيرفر
  const channel = member.guild.channels.find('id', '520940148620263426'); //ايدي الروم
if (!channel) return;
channel.send(`**<@${member.user.id}> Welcome To Lwa  ** ❤️ `)  
}});
client.on("guildMemberRemove", member => {
        if(member.guild.id === "520940148620263424") { 
  const channel = member.guild.channels.find('id', '520940148620263426'); 
if (!channel) return;
...
client.login('NTIwOTI4OTI0NTkxNzE4NDAw.Du5wbg.vCEiv3DLZQQxGUIga-hPUeio_C8');
